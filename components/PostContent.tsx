...
const PostContent = ({ postTitle } : { postTitle: string }) => {
  ...
  const postTitleWithSpaces = postTitle;
  ...
  async function createAnswerPost(data: Input) {
    ...
    const postWithId = doc(queRef, postTitleWithSpaces);
    const post = await getDoc(postWithId);
    console.log("Post with id: ", post.data());
    ...
    if (post.exists()) {
      const doc = {id : post.id, ...post.data()}; // Get the first (and should be only) document that matches the query
      ...
      await setDoc(newNotificationRef, {
        ...
        answerUid: user?.uid,
        answerName: name||user?.name||fetchedUser.name||fetchedUser.displayName,
  ...
  useEffect(() => {
    ...
    const Func = async () => {
      const queRef = collection(db, "questions");
      // assume that postTitleWithSpaces is id for that question now find that by it
      const postWithId = doc(queRef, postTitleWithSpaces);
      const post = await getDoc(postWithId);
      console.log("Post with id: ", {id : post.id, ...post.data()});
        if (postWithId) {
          const doc = post;
          ...
          const qAns = query(ansRef, where("questionId", "==", postTitleWithSpaces)); // Order by 'createdAt' in descending order
  ...
  return (
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-7 gap-y-4 md:gap-x-4 py-6 px-3 font-dmsans mt-3">
      ...
      <div className={`${ansLoading?"overflow-auto":""} `}>
        ...
        <div className=" mt-3">
          {isCommentBoxOpen ? (
            <div
              className="rounded-3xl border border-gray-500 p-4 cursor-auto mx-2 md:mx-0 my-6"
              onClick={() => setIsCommentBoxOpen(false)}
            >
              Write a Answer...
            </div>
          ) : (
            <div className="bg-white dark:text-white dark:bg-[#262626] rounded-3xl border border-gray-500 p-4 cursor-pointer">
              <Form {...form}>
                <form
