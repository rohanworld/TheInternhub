...
const RecentFeedCard = ({ post }: Props) => {
  ...
  <div className='flex max-h-40 mt-1 space-x-3 text-gray-500 dark:text-yellow-50'>
        ...
        <span className='font-style-5-username font-dmsans '>{isAnonymous ? 'Anonymous' : post.name}</span>
          </div>
    <div className="flex flex-col">
    <Link href={`/${post.title.split(' ').join('-')}`}>
      <h3 className="font-style-2 font-dmsans  mb-1">{post.title.length>27?`${post.title.substring(0,27)}...`:post.title}</h3>
    </Link>
      <div className="text-sm flex gap-1">
        <p className="text-zinc-500 font-style-4 font-dmsans ">{post.voteAmt?post.voteAmt:0} supports</p>
        ...
        <p className="text-zinc-500 font-style-4 font-dmsans ">{post.comments} answers</p>
      </div>
    </div>
  </div>
</div>
