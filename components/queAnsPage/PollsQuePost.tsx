...
const [pollData, setPollData] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchPollData = async () => {
      if (post?.id) { // Check if post.id exists
        try {
      ...
      <div className="px-6 md:py-6 py-4 flex justify-between">
        ...
        <div className="w-0 flex-1">
          <div className="flex justify-between max-h-60 mt-1 space-x-2 text-xs">
            ...
            <Avatar>
              <div className=" relative w-full h-full aspect-square">
                <Image
                  fill
                  src={isAnonymous ? ('https://e7.pngegg.com/pngimages/416/62/png-clipart-anonymous-person-login-google-account-computer-icons-user-activity-miscellaneous-computer.png') : (post.profilePic ? post.profilePic : '/anonymous2.png')}
                  alt="profile picture"
          ...
          <div>
            ...
            <div>
              {
                (user.userType==UserType.Guest)?<Button className={`rounded-3xl border-2 text-center font-style-3-vote font-jakarta border-black h-[2rem] bg-transparent text-black hover:text-black hover:bg-[#b6b8bd]`} onClick={() => {toast({
