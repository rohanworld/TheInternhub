...
const PollsCommentPost = ({
  ...
  async function createCommentPost(data: Input) {
    ...
    const FindPollById = await getDoc(doc(db, "polls", postTitleWithSpaces));
    ...
    if (FindPollById.exists()) {
      const doc = FindPollById.data(); // Get the first (and should be only) document that matches the query
      ...
      const docRef = await addDoc(
        collection(db, "pollsAnswers", answerId, "comments"),
        {
          ...
          uid: user?.uid,
          name: user?.displayName||fetchedUser.name||fetchedUser.displayName,
  ...
  async function submitReply(index: number) {
    ...
    const reply: ReplyType = {
      reply: replyText,
      name: name||user?.displayName || fetchedUser.displayName||fetchedUser.name,
      profilePic: user?.photoURL || fetchedUser.profilePic || "https://med.gov.bz/wp-content/uploads/2020/08/dummy-profile-pic.jpg",
    ...
    if (FindPollById.exists()) {
      const questionDoc = FindPollById.data(); // Get the first (and should be only) document that matches the query
