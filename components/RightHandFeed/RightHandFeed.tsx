          ...
          <Card x-chunk="dashboard-01-chunk-5" className='rounded-2xl'>
              ...
              <CardContent className="mt-2">
                ...
                <Link href={'/createQue'} className='w-full'>
                  <Button className=' w-full font-style-4 font-dmsans '><a target='_blabk' href="#">Ask Question</a></Button>
                  </Link>
                </div>
              </CardContent>
          </Card>
      ...
      <TableBody>
        {posts.map((post, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="font-style-2 font-dmsans  font-[500]">{post.title.length>70?post.title.substring(0, 69)+"...":post.title}</TableCell>
            <TableCell className="font-style-1-descriptions font-jakarta  text-zinc-500 dark:text-gray-400">{post.description.length>1000?parse(post.description.substring(0, 99))+"...":parse(post.description)}</TableCell>
            </Link>
            {
              index!=4&&
            <Separator/>
