...
const ForumAnsPost = ({answers , postTitleWithSpaces , postId }: Props) => {
  ...
  useEffect(() => {
    const fetchFollowedUsers = async () => {
      if (!user || !user.uid) return; // Add a check for user.uid
  
      setLoadingFollowedUsers(true);
      try {
        const userRef = doc(db, 'users', user.uid); // Ensure user.uid is not undefined
  ...
  const followUser = async (userId: string) => {
    ...
    // Ensure user.uid is defined before proceeding
    if (!user.uid) {
    ...
    try {
      const userRef = doc(db, 'users', user.uid); // Make sure user.uid is defined
      await updateDoc(userRef, {
        following: arrayUnion(userId),
      });
  ...
  const unfollowUser = async (userId: string) => {
    ...
    if (!user || user.userType === "guest") {
      toast({
        title: "Please login to unfollow others",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    ...
    <div className=" mt-3 font-dmsans">
      {dispAnswer.length>0&&dispAnswer.map((answer: any, key) => (
        ...
        <div
          key={answer.id}
          className="rounded-2xl bg-white dark:bg-gray-800 my-3 space-y-0 break-words overflow-hidden shadow-[0px_0px_0px_1px_rgba(8,112,184,0.06),0px_1px_1px_-0.5px_rgba(8,112,184,0.06),0px_3px_3px_-1.5px_rgba(8,112,184,0.06),_0px_6px_6px_-3px_rgba(8,112,184,0.06),0px_12px_12px_-6px_rgba(8,112,184,0.06),0px_24px_24px_-12px_rgba(8,112,184,0.06)]" 
        >
