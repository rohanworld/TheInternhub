...
// }, [post.uid, user]);
useEffect(() => {
  // Ensure both user and post.uid are defined
  if (user?.uid && post?.uid) {
    ...
    const unsubscribe = onSnapshot(userRef, (docSnapshot) => {
      if (docSnapshot.exists()) {
        ...
        // Update the state for whether the current user is following the post's creator
        setIsFollowing(userData?.following?.includes(post.uid) || false);
        
        // Update the state to check if the current user is the post's creator
...
const handleFollow = async () => {
  ...
  // Ensure both user.uid and post.uid are defined before proceeding
  if (!user.uid || !post?.uid) {
    console.error("User ID or Post ID is undefined.");
    return;
  }
  ...
  try {
    ...
    // Update the followers list of the post's creator
...
let dateString;
  ...
  return (
    ...
    <div className="rounded-2xl bg-white dark:bg-gray-800 break-words overflow-hidden mt-1 shadow-[0px_0px_0px_1px_rgba(8,112,184,0.06),0px_1px_1px_-0.5px_rgba(8,112,184,0.06),0px_3px_3px_-1.5px_rgba(8,112,184,0.06),_0px_6px_6px_-3px_rgba(8,112,184,0.06),0px_12px_12px_-6px_rgba(8,112,184,0.06),0px_24px_24px_-12px_rgba(8,112,184,0.06)]">
      <div className="px-6 md:py-6 py-4 flex justify-between">
        ...
        <div className="w-0 flex-1">
          ...
          {/* <a href={`/postPage/${post.id}`}> */}
            <h1 className="text-xl py-3 leading-6">
              {post.title}
            </h1>
          {/* </a> */}
