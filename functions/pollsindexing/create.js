...
const ALGOLIA_INDEX_NAME = 'search_polls';

const client = algoliasearch(ALGOLIA_APP_ID, ALGOLIA_ADMIN_KEY);
const index = client.initIndex(ALGOLIA_INDEX_NAME);

exports.onQuestionCreated = functions.firestore.document('polls/{pollId}').onCreate((snap, context) => {
...
exports.onQuestionUpdated = functions.firestore.document('polls/{pollId}').onUpdate((change, context) => {
...
exports.onQuestionDeleted = functions.firestore.document('polls/{pollId}').onDelete((snap, context) => {
    const objectID = snap.id;
...
exports.deleteOldPosts = functions.pubsub.schedule('every 2 minutes').onRun(async context => {
    ...
    const recentPostsQuery = admin.firestore().collection('polls').where('createdAt', '>=', twoMinutesAgo);
